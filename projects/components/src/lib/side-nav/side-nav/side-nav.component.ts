...
export class SideNavComponent implements OnInit {
  ...
  noCollapseMenuItems: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.sidenav.collapsed.subscribe(
    ...
    if (this.url?.includes('home') || this.url?.includes('support')) {
  ...
  getActiveClass(data: INavbarData): string {
    if (this.collapsed) {
      return this.url === data?.routeLink
        ? 'side-panel-body-content-link-collapsed-active'
        : '';
    } else {
      return this.url === data?.routeLink
        ? 'side-panel-body-content-link-active'
        : '';
    }
  }
  ...
  selectNav(menu: any) {
    if (menu?.items?.length === 0 || !menu?.items) {
      if (
        menu.routeLink === this.menus[0].routeLink ||
        menu.routeLink === this.menus[5].routeLink
      ) {
        this.collapsed = false;
        this.noCollapseMenuItems = true;
        this.sidenav.collapseNav(this.collapsed);
      } else {
